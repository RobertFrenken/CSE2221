...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        // start with the start of string 2, and the end of string 1
        int i = str1.length() - overlap;
        // take all of string one, and the difference of string 2 and the overlap attached to the end
        String combination = str1.substring(0, i) + str2;
        return combination;
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        if (!isSubstring) {
            ...
            // loop whole set except for last entry, which was new string, and remove substrings of addition
            while (strSet.size() != 0) {
                String single = strSet.removeAny();
                if (str.indexOf(single) == -1) {
                    newSet.add(single); // add if not a substring
                }
            }
            strSet.add(str); // add string
            strSet.add(newSet); // add entries that are not substring
        }
    }
