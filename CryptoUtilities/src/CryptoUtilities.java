...
public final class CryptoUtilities {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        NaturalNumber k = n.newInstance();
        k.copyFrom(n);
        NaturalNumber two = n.newInstance();
        two.setFromInt(2);
        boolean returnValue;

        NaturalNumber j = k.divide(two);
        if (j.isZero()) {
            returnValue = true;
        } else {
            returnValue = false;
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (p.isZero()) {
            ...
            if (isEven(p)) {
                NaturalNumber remainderPower = p.divide(two);
                powerMod(n, p, m);
                p.multiply(two);
                p.add(remainderPower);
                NaturalNumber intermediate = n.newInstance();
                intermediate.add(n);
                n.multiply(intermediate);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        // check statement one from theorem
        ...
        // check statement two from theorem
        powerMod(newW, power, newN);
        if (newW.compareTo(one) != 0) {
            isComposite = true;
        }

        // if code enters neither loop, "likely" to be a prime number
        return isComposite;
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        for (int i = 0; i < 50; i++) {
            if (isPrime) {
                NaturalNumber one = new NaturalNumber2(1);
                NaturalNumber boundary = new NaturalNumber2(n);
                boundary.decrement();
                NaturalNumber w = randomNumber(n);
                while (w.compareTo(one) <= 0) {
                    w.increment();
                }
                while (w.compareTo(boundary) >= 0) {
                    w.decrement();
                }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        boolean isLikelyPrime = isPrime2(n);
        while (!isLikelyPrime) {
            n.add(two);
            isLikelyPrime = isPrime2(n);
        }
